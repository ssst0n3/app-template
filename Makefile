.PHONY: all shell binary build image install
...
install: build
	rm -f /usr/local/bin/${APP_NAME} && ln -s $(CURDIR)/bin/latest/${APP_NAME}_linux_amd64 /usr/local/bin/${APP_NAME}
